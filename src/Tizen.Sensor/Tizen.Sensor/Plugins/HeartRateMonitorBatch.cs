...
{
    /// <summary>
    /// The HeartRateMonitorBatch class is used for TODO.
    /// </summary>
    ...
    public sealed class HeartRateMonitorBatch : BatchSensor<HeartRateMonitorBatchData>
    {
        ...
        protected override IReadOnlyList<HeartRateMonitorBatchData> ConvertBatchData()
        {
            ...
            for (int i = 0; i < BatchedEvents.Count; i++)
            {
                ...
                list.Add(new HeartRateMonitorBatchData(new TimeSpan((Int64)sensorData.timestamp), sensorData.accuracy, (HeartRateMonitorBatchState)sensorData.values[0], (int)sensorData.values[1], (int)sensorData.values[2]));
            }
